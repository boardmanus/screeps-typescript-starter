...
function unload_at_site(job : JobUnload, worker : Creep, site : UnloadSite) : Operation {
  return () => {
    ...
    switch (res) {
      default:
        log.error(`${job.id()}: unexpected error while ${worker} unloaded at ${site} (${u.errstr(res)})`);
        break;
      case ERR_NOT_IN_RANGE: {
        const res = worker.moveTo(site);
          if (res == OK) {
            log.info(`${job.id()}: ${worker} moved towards unload site ${site}`);
          }
          else {
            log.warning(`${job.id()}: ${worker} failed moving to unload ${site} (${u.errstr(res)})`);
          }
        }
        break;
      case OK:
        // Finished job.
        log.info(`${job.id()}: ${worker} transferred energy to ${site}`);
...
type UnloadSite = Extension | Spawn | Storage | Container | Link | Creep;
