...
function is_mineral_structure(s: Structure | ConstructionSite): boolean {
...
function find_mine_structures(mine: Mineral): AnyStructure[] {
  return mine.pos.findInRange(FIND_STRUCTURES, 1, { filter: is_mineral_structure }) ?? [];
}

function find_mine_construction(mine: Mineral): ConstructionSite[] {
  return mine.pos.findInRange(FIND_CONSTRUCTION_SITES, 1, { filter: is_mineral_structure }) ?? [];
}
...
function container_building_work(mine: Mineral): WorkBuilding | undefined {
...
export default class BusinessMineralMining implements Business.Model {
  ...
  constructor(mine: Mineral, priority = 5) {
  ...
  buildings(): WorkBuilding[] {
    ...
    if (!mine._extractor && can_build_extractor(mine)) {
      work.push(new WorkBuilding(mine.pos, STRUCTURE_EXTRACTOR));
    }
